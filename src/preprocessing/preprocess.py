...
if not os.path.exists(path.replace("nii", "npy")):
    ...
    def remove_items(slice, rr):
        # Apply radial filter to remove sample holder
    ...
    print("Radial Filtration Complete")
    
    #Median filter twice to make finding the ideal threshold easier.
    print("Median Filtering...")
    masked_slices = median_filter(masked_slices, footprint=np.ones((3,3,3)))
    print("First Median Filtration Complete")
    masked_slices = median_filter(masked_slices, footprint=np.ones((3,3,3)))
    print("Second Median Filtration Complete")
    
    print("Segmenting...")
    thresh1 = filters.threshold_multiotsu(masked_slices[masked_slices != DEFAULT_VAL], classes=3)[0] # Double check this line at another point, the background should already be zeroed so this line may be segmenting some peg to background
    foreground = masked_slices[masked_slices > thresh1]
    print("Background Segmented")
    thresh2 = filters.threshold_triangle(foreground)
    regions = np.digitize(masked_slices, bins=[thresh1, thresh2])
    print("API Segmented")
    
    np.save(path.replace("nii", "npy"), regions)
    # os.remove(path.replace("nii", "npz"))
else:
    regions = np.load(path.replace("nii", "npy"))

plt.imshow(regions[10])
plt.show()
plt.imshow(regions[:, 375])
plt.show()
plt.imshow(regions[:, np.random.choice(regions.shape[1])])
plt.show()

# Count how much API is calculated to be in the capsule
